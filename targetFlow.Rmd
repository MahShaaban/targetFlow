...
```{r foldchange,fig.height=3,fig.cap='Differential expression between factor knockdown and wild-type in HeLa cells.'}
par(mfrow = c(1, 3))
plot(express$YY1$fc, 
...
```{r functions,fig.cap='Predicted functions of YY1 and YY2 on their specific targets.'}
par(mfrow = c(2, 2))
map(ap, ~plot(.x$distance, 
              .x$peak_score,
              xlab = 'Distance',
              ylab = 'Peak Score'))
...
map2_df(dt, groups,
     ...
     .id = 'Factor') %>%
  knitr::kable(
    caption = 'Testing for statistical significance of the regulated gene groups.',
    label = 'tests')
```
...
```{r function,fig.height=3,fig.cap='Predicted function of YY1 and YY2 on their shared targets.'}
par(mfrow = c(1, 2))
...
```{r occurences,fig.height=3,fig.cap="Occurences of consensus sequences in the ranked regions."}
par(mfrow = c(1, 2))
map(bcout, ~plot(toptable(.x, 1)))
```

Figure \@ref(fig:motifs)

```{r eval=FALSE}
map(bcout, ~seqLogo(pwm(toptable(.x, 1))))
```

```{r motifs,fig.height=3,fig.cap="Predicted motifs of the cooperative and competitive binding sites."}
